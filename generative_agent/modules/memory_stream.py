...
class MemoryStream: 
  def __init__(self, nodes = [], embeddings = {}): 
    ...
    # self.last_node_index = len(self.seq_nodes) - 1


  def package(self):
    """
    Packaging the agent's memory stream for saving.

    Parameters:
      None
    Returns:
      A packaged dictionary of the memory stream information
    """
    return {"nodes": [node.package() for node in self.seq_nodes],
  ...
  def retrieve(self, focal_points, time_step, n_count=8, curr_filter="all",
  ...
  def forget_all(self):
    """
    Clear the memory stream by removing all nodes.
  ...
  def forget(self, memory_id: Union[int, List[int]]):
    """
    Remove a specific memory record from the memory stream.

    Parameters:
      memory_id: The ID of the memory record to be removed, or a list of IDs.
    Returns:
      None
    """
  
    memory_ids = memory_id if type(memory_id) == list else [memory_id]
    for memory_id in memory_ids:
      if memory_id in self.id_to_node:
          node = self.id_to_node[memory_id]
          self.seq_nodes.remove(node)
          del self.id_to_node[memory_id]
          del self.embeddings[node.content]

      ## reindex the nodes
    for count, node in enumerate(self.seq_nodes): 
        node.node_id = count
  ...
  def get_memories(self, n_count=10, curr_filter="all", hp=[1, 0, 0]): 
    """
    TODO: Fix this method to actually use all of the inputs.
    Retrieve elements from the memory stream. 
    ...
    for node in self.seq_nodes:
        memories[node.node_id] = node.content

    last_8_nodes = sorted(memories.keys())[-n_count:]
    return {node_id: memories[node_id] for node_id in last_8_nodes}
